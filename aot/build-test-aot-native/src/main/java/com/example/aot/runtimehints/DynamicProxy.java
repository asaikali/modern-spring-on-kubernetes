...
public class DynamicProxy implements HelloService {
  ...
  public String sayHello(String name) {
    // A proxy instance serviced by an invocation handler is created via a factory method call on
    // the java.lang.reflect.Proxy class
    // Since InvocationHandler is a functional interface, let's the handler inline using lambda
    // expressions
    Map proxyInstance;
    try {
      proxyInstance =
    ...
    try {
      ...
      System.out.println(
          "DynamicProxy: Expected an exception, got an exception: put is not supported in the proxy.");
    }
