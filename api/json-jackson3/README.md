# Jackson 3 JSON Processing Demo

This project demonstrates comprehensive **Jackson 3** (`tools.jackson`) JSON processing capabilities through JUnit tests and a Spring Boot REST API. Jackson 3 is the default JSON library in Spring Boot 4; it showcases advanced Jackson features for JSON serialization, deserialization, and configuration in real-world scenarios.

See the sibling [json-jackson2](../json-jackson2/README.md) module for the same samples on Jackson 2 (`com.fasterxml.jackson`), and compare the two side by side to see exactly what changed between the generations (immutable builder-based mappers, unchecked exceptions, ValueSerializer/ValueDeserializer, built-in java.time support).
