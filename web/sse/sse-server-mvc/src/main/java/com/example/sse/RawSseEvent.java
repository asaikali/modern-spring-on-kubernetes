...
/**
 * Immutable holder of the raw text of one SSE event— exactly as it arrived from the stream.
 *
 * <p>Follows WHATWG HTML Living Standard § 9.2 Server-sent events
...
public record RawSseEvent(String text) {
  ...
  /**
   * Holder for all parsed SSE fields per the spec. - data: lines joined with '\n', trailing newline
   * removed (§ 9.2.6) - comments: lines joined with '\n', trailing newline removed
   *
   * <p>See WHATWG HTML § 9.2.6 "Interpreting an event stream"
   */
  public record Fields(String id, String event, Duration retry, String data, String comment) {}

  /**
   * Parse the raw event text into its constituent fields per SSE spec.
   *
   * <p>Implements the field processing algorithm from WHATWG HTML § 9.2.6 "Interpreting an event
   * stream" - specifically the steps for processing fields after line parsing.
   */
  public Fields parseFields() {
    ...
    String data = dataBuffer.length() > 0 ? dataBuffer.substring(0, dataBuffer.length() - 1) : "";
    String comment =
        commentBuffer.length() > 0 ? commentBuffer.substring(0, commentBuffer.length() - 1) : null;
   ...
   *
   * <p>Utility method for serialization/deserialization scenarios. Note: The SSE spec (§ 9.2.5)
   * mandates UTF-8 encoding for event streams.
   *
