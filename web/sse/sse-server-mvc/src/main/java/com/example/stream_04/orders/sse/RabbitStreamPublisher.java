 ...
 * acknowledged by RabbitMQ. Order and in-order delivery is very important
 * for this use case.
 * 
 * <p>In this example scenario, we are dealing with an upstream service that produces
 * a set of events that we want to capture. We want those events to be published
 * in sequence to the stream and not out of order. This is why we need confirmation
...
public class RabbitStreamPublisher implements AutoCloseable {
   ...
   * 
   * <p>This method serializes the provided object to JSON, creates a message with
   * appropriate properties, sends it to the stream, and waits for confirmation
   * from RabbitMQ before returning.
   *
   ...
   * @return {@code true} if the message was successfully confirmed by RabbitMQ, {@code false} otherwise
   ...
   * 
   * <p>This method should be called when the publisher is no longer needed
   * to release resources and properly clean up the connection to RabbitMQ.
   *
