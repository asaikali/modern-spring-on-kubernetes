 ...
 * <ul>
 * <li>{@link Immediate}: For responses where all necessary data is available
 * and returned immediately.</li>
 * <li>{@link Stream}: For responses that initiate or refer to a continuous
...
public sealed interface ApiResponse permits ApiResponse.Immediate, ApiResponse.Stream {
  ...
  /**
   * <p>Represents an immediate, synchronous data response from an API operation.
   * This record is used when the entire result of an API operation can be delivered
  ...
  /**
   * <p>Represents an API response that provides a reference to a Server-Sent Events (SSE) stream.
   * This record is used when the result of an API operation involves continuous updates or
