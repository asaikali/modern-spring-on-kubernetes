...
/**
 * A simple SSE client using RestClient. Delegates stream processing to SseStreamProcessor for clean
...
public class SseClient {
  ...
  /**
   * Subscribe to the given SSE URI. For each event, your handler is called with a RawSseEvent
   * containing the raw text.
   *
   * <p>Uses SseStreamProcessor to handle the actual stream parsing, while this class focuses on
        ...
        .exchange(
            (req, resp) -> {
              try (InputStream is = resp.getBody();
                  SseStreamProcessor processor = new SseStreamProcessor(is, handler, uri)) {
